        ...
        if (declaration.type === 'declaration' && pxRegExp.test(declaration.value)) {
          ...
          if (nextDeclaration && nextDeclaration.type === 'comment') { // next next declaration is comment
            if (nextDeclaration.comment.trim() === config.forcePxComment) { // force px
              ...
              declarations.splice(j + 1, 1); // delete corresponding comment
            } else if (config.keepValue && new RegExp(config.keepValue).test(declaration.value)) {
              // value match user's options, keep value
              continue;
            } else if (config.keepProperty && new RegExp(config.keepProperty).test(declaration.property)) {
